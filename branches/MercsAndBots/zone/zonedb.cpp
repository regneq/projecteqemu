...
/*void ZoneDatabase::LoadMercTypes(){
	...
	if(!database.RunQuery(Query, MakeAnyLenString(&Query, "SELECT `dbstring` AS `type`, `clientversion` FROM `merc_types` order by `race_id`, `proficiency_id`"), TempErrorMessageBuffer, &DatasetResult)) {
		...
		while(DataRow = mysql_fetch_row(DatasetResult)) {
			...
			merc_types.push_back(tempMercType);
		}
	...
	if(!errorMessage.empty()) {
		LogFile->write(EQEMuLog::Error, "Error in ZoneDatabase::LoadMercTypes()");
	}
}*/

void ZoneDatabase::LoadMercTemplates(){
