...
int IRC::message_loop()
{
	...
	_log(IRC__TRACE, "IRC::message_loop(): starting the loop");
	while (1)
	{
		ret_len=recv(irc_socket, buffer, 1023, 0);
		if (ret_len==SOCKET_ERROR || !ret_len)
		{
			_log(IRC__TRACE, "IRC::message_loop(): ending the loop (ret_len: %i)", ret_len ? ret_len : 0);
			return 1;
		}
		buffer[ret_len]='\0';
		_log(IRC__TRACE, "IRC::message_loop(): splitting replies");
		split_to_replies(buffer);
	}
	_log(IRC__TRACE, "IRC::message_loop(): ending the loop (!1)");
...
void IRC::split_to_replies(char* data)
{
	...
	{
		*p='\0';
		_log(IRC__TRACE, "IRC::split_to_replies(): parsing the reply");
		parse_irc_reply(data);
		data=p+2;
	}
	_log(IRC__TRACE, "IRC::split_to_replies(): finished parsing replies");
}
