...
//
// This method returns the number of entries in the hate list that match the boolean parameters
//
// CountGreens:         If False, do not count mobs that con green to hater
// CountFearedOrMezzed: If False, do not count mobs that are currently feared, fleeing or mezzed
// RangeCheck:          If True, only count mobs for whom  hater is in their aggro radius
//
int32 HateList::GetHatedCount(Mob *hater, bool CountGreens, bool CountFearedOrMezzed, bool RangeCheck) {

	_ZP(HateList_GetHatedCount);

	// Count of mobs on this hate list that meet the criteria
	int32 Count = 0;
	...
	while(iterator.MoreElements()) {
		...
		Mob* HatedMob = iterator.GetData()->ent;
        	iterator.Advance();

		if(HatedMob != NULL) {

			if( !CountFearedOrMezzed && (HatedMob->IsFeared() || HatedMob->IsMezzed())) continue;

			if(!CountGreens)
				if(hater->GetLevelCon(HatedMob->GetLevel()) == CON_GREEN) continue;

			// Get the aggro radius of the hated mob
			
			float AggroRange = HatedMob->GetAggroRange();

			// Square it because we will be using DistNoRoot
			
			AggroRange = AggroRange * AggroRange;

			if(RangeCheck && (HatedMob->DistNoRoot(*hater) > AggroRange)) continue;
