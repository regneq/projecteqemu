...
static inline structs::VoASlotStruct TitaniumToVoASlot2(int32 TitaniumSlot)
{
	...
	if((TitaniumSlot >= 22) && (TitaniumSlot <= 29))
	{
		...
		//_log(NET__ERROR, "Main Inv TitaniumSlot is %i, VoASlot is %i, %i", TitaniumSlot, VoASlot.MainSlot, VoASlot.SubSlot);
		return VoASlot;
	}
	...
	{
		VoASlot.Bank = 1;
		VoASlot.MainSlot = TitaniumSlot - 2000;
		VoASlot.SubSlot = 0xffff;
	...
	{
		VoASlot.Bank = 1;
		VoASlot.MainSlot = (TitaniumSlot - 2031) / 10;
		VoASlot.SubSlot = (TitaniumSlot - 2031) - (VoASlot.MainSlot * 10);
		//_log(NET__ERROR, "Converted bank bag slot %i to %i, %i", TitaniumSlot, VoASlot.MainSlot, VoASlot.SubSlot);
	...
	//_log(NET__ERROR, "HotSlot is %i, VoASlot is %i, %i", TitaniumSlot, VoASlot.MainSlot, VoASlot.SubSlot);
...
static inline int32 VoAToTitaniumSlot2(int32 VoASlot) {
	...
	{
		TitaniumSlot = VoASlot - 1;
	}
	else if(VoASlot >= 262 && VoASlot <= 351)	// Bag Slots for Normal Inventory and Cursor 
	{
		TitaniumSlot = VoASlot - 11;
	}
	else if(VoASlot >= 2032 && VoASlot <= 2271)	// Bank Bag Slots
	{
		TitaniumSlot = VoASlot - 1;
	}
	else if(VoASlot >= 2532 && VoASlot <= 2551)	// Shared Bank Bag Slots
	{
		TitaniumSlot = VoASlot - 1;
	}
	else if(VoASlot == 21)
...
ENCODE(OP_OpenNewTasksWindow) {
	...
	*p = NULL;
	...
	for(uint32 i=0; i<__emu_AvailableTaskHeader->TaskCount; i++) {
		...
		// in VoA packets. Changing it to 0x3f000000 makes the title red.
...
ENCODE(OP_SendCharInfo) {
	...
	for(r = 0; r < char_count; r++) {
		...
		{	//post-name section...
			...
			// Temp for VoA March 15th 2012 client
			eq2->char_enabled = 1;
	...
	_log(NET__ERROR, "Sending OP_ZoneEntry");
		...
		*p = NULL;
		...
		for(r = 0; r < entrycount; r++, emu++) {
			...
			VARSTRUCT_ENCODE_TYPE(uint8, Buffer, 0);  // VoA unkown TEST
...
DECODE(OP_AdventureMerchantSell) {
	...
	IN(npcid);
	emu->slot = VoAToTitaniumSlot(eq->slot);
	IN(charges);
	IN(sell_price);
...
DECODE(OP_ReadBook) {
	...
	IN(type);
	emu->invslot = VoAToTitaniumSlot(eq->invslot);
...
char* SerializeItem(const ItemInst *inst, sint16 slot_id_in, uint32 *length, uint8 depth) {
	...
	VoA::structs::ItemSerializationHeader hdr;
	...
	ss.write((const char*)&hdr, sizeof(VoA::structs::ItemSerializationHeader));
	...
	ss.write((const char*)&null_term, sizeof(uint8));
	//_log(NET__ERROR, "ItemBody struct is %i bytes", sizeof(VoA::structs::ItemBodyStruct));
	VoA::structs::ItemBodyStruct ibs;
	memset(&ibs, 0, sizeof(VoA::structs::ItemBodyStruct));
	...
	ss.write((const char*)&ibs, sizeof(VoA::structs::ItemBodyStruct));
	...
	//_log(NET__ERROR, "ItemBody secondary struct is %i bytes", sizeof(VoA::structs::ItemSecondaryBodyStruct));
	VoA::structs::ItemSecondaryBodyStruct isbs;
	memset(&isbs, 0, sizeof(VoA::structs::ItemSecondaryBodyStruct));
	...
	ss.write((const char*)&isbs, sizeof(VoA::structs::ItemSecondaryBodyStruct));
	...
	//_log(NET__ERROR, "ItemBody tertiary struct is %i bytes", sizeof(VoA::structs::ItemTertiaryBodyStruct));
	VoA::structs::ItemTertiaryBodyStruct itbs;
	memset(&itbs, 0, sizeof(VoA::structs::ItemTertiaryBodyStruct));
	...
	//_log(NET__ERROR, "ItemBody Click effect struct is %i bytes", sizeof(VoA::structs::ClickEffectStruct));
	VoA::structs::ClickEffectStruct ices;
	memset(&ices, 0, sizeof(VoA::structs::ClickEffectStruct));
	...
	ss.write((const char*)&ices, sizeof(VoA::structs::ClickEffectStruct));
	...
	//_log(NET__ERROR, "ItemBody proc effect struct is %i bytes", sizeof(VoA::structs::ProcEffectStruct));
	VoA::structs::ProcEffectStruct ipes;
	memset(&ipes, 0, sizeof(VoA::structs::ProcEffectStruct));
	...
	ss.write((const char*)&ipes, sizeof(VoA::structs::ProcEffectStruct));
	...
	//_log(NET__ERROR, "ItemBody worn effect struct is %i bytes", sizeof(VoA::structs::WornEffectStruct));
	VoA::structs::WornEffectStruct iwes;
	memset(&iwes, 0, sizeof(VoA::structs::WornEffectStruct));
	...
	ss.write((const char*)&iwes, sizeof(VoA::structs::WornEffectStruct));
	...
	VoA::structs::WornEffectStruct ifes;
	memset(&ifes, 0, sizeof(VoA::structs::WornEffectStruct));
	...
	ss.write((const char*)&ifes, sizeof(VoA::structs::WornEffectStruct));
	...
	VoA::structs::WornEffectStruct ises;
	memset(&ises, 0, sizeof(VoA::structs::WornEffectStruct));
	...
	ss.write((const char*)&ises, sizeof(VoA::structs::WornEffectStruct));
	...
	// Bard Effect?
	VoA::structs::WornEffectStruct ibes;
	memset(&ibes, 0, sizeof(VoA::structs::WornEffectStruct));
	...
	ss.write((const char*)&ibes, sizeof(VoA::structs::WornEffectStruct));
	...
	//_log(NET__ERROR, "ItemBody Quaternary effect struct is %i bytes", sizeof(VoA::structs::ItemQuaternaryBodyStruct));
	VoA::structs::ItemQuaternaryBodyStruct iqbs;
	memset(&iqbs, 0, sizeof(VoA::structs::ItemQuaternaryBodyStruct));
	...
	ss.write((const char*)&iqbs, sizeof(VoA::structs::ItemQuaternaryBodyStruct));
